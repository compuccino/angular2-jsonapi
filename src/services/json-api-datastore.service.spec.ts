...
describe('JsonApiDatastore', () => {
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [Datastore, DatastoreWithConfig]
    });
  ...
  describe('query', () => {
    it('should build basic url from the data from datastore decorator', () => {
      ...
      const expectedUrl = `${BASE_URL}/${API_VERSION}/${
        authorModelConfig.type
      }`;
    ...
    it('should build basic url and apiVersion from the config variable if exists', () => {
      ...
      const expectedUrl = `${BASE_URL_FROM_CONFIG}/${API_VERSION_FROM_CONFIG}/${
        authorModelConfig.type
      }`;
    ...
    it('should have custom headers', () => {
      ...
      datastore
        .query(Author, null, new Headers({ Authorization: 'Bearer' }))
        .subscribe();
    ...
    it('should override base headers', () => {
      ...
      datastore
        .query(Author, null, new Headers({ Authorization: 'Basic' }))
        .subscribe();
  ...
  describe('saveRecord', () => {
    it('should create new author', () => {
      ...
      expect(obj.attributes.dob).toEqual(
        format(parse(AUTHOR_BIRTH), 'YYYY-MM-DDTHH:mm:ssZ')
      );
  ...
  describe('updateRecord', () => {
    it('should update author with 200 response (no data)', () => {
      ...
      expect(obj.attributes.dob).toEqual(
        format(parse('1965-07-31'), 'YYYY-MM-DDTHH:mm:ssZ')
      );
