...
export class JsonApiModel {
  ...
  save(params?: any, headers?: Headers): Observable<this> {
    ...
    return this._datastore.saveRecord(
      attributesMetadata,
      this,
      params,
      headers
        ...
        if (
          ...
          if (!includes(modelTypesFetched, typeName)) {
            ...
            const modelType: ModelType<this> = Reflect.getMetadata(
              ...
              this._datastore.constructor
            ).models[typeName];

            if (modelType) {
              // tslint:disable-next-line:max-line-length
              this[metadata.propertyName] = this.getHasOneRelationship(
                modelType,
                relationship.data,
                included,
                typeName,
              ...
              throw {
                message:
                  'parseHasOne - Model type for relationship ' +
                  typeName +
                  ' not found.'
              };
            }
          ...
          for (const typeIndex of Object.keys(relationship.data)) {
            ...
            if (!includes(modelTypesFetched, typeName)) {
              ...
              if (modelType) {
                ...
                throw {
                  message:
                    'parseHasMany - Model type for relationship ' +
                    typeName +
                    ' not found.'
                };
              }
        ...
        if (relationship && relationship.data) {
          ...
          if (dataRelationship) {
            ...
            if (modelType) {
              ...
              throw {
                message:
                  'parseBelongsTo - Model type for relationship ' +
                  typeName +
                  ' not found.'
              };
            }
